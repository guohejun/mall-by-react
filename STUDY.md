...
2. 安装react-app-rewired
  ...
  ##### 8.架构项目需要注意至少完备哪些功能点？
  01. 【√】请求封装，请求拦截，响应拦截，拦截信息提示。
  02. 【√】字体图标使用，具体采用哪种方案（配置svg-loader、封装icon组件等）。
  03. 【√】移动端封装长列表无限加载、下拉刷新组件；
  04. 【√】配置资源文件路径别名（alias）。
  05. 【√】移动端页面尺寸处理（rem或vw控制）。
  06. 【√】样式重置（区分PC和移动端）（reset.css或叫normal.css）。
  07. 【√】html页面meta属性设置（区分PC和移动端）。
  08. 【√】公共API封装。
  09. 【?】状态管理全局配置（Vue的Vuex或react的react-redux）。
  10. 【√】每个项目根目录创建Study.md文件，总结问题，积累经验。
  11. 【?】★★★★★路由懒加载。
  12. 【?】★★★★路由拦截，场景：未登录时，跳转至登录页面。
